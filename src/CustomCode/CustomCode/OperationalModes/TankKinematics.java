 ...
 */
@Config
public class TankKinematics extends TankDrive {
    public static PIDCoefficients AXIAL_PID = new PIDCoefficients(8, 0, 0);
    public static PIDCoefficients CROSS_TRACK_PID = new PIDCoefficients(3, 0, 0);
    ...
    public SampleTankDrive(HardwareMap hardwareMap) {
        ...
        follower = new TankPIDVAFollower(AXIAL_PID, CROSS_TRACK_PID,
                new Pose2d(0.5, 0.5, Math.toRadians(1.0)), 1.5);
...
//        }
    ...
    public static String outputable;

    @Override
    public void setDrivePower(Pose2d pose)
    {
        outputable = "x: " + pose.getX() + "    h: " + turnToSteer(pose.getHeading());

        steer.setPosition(turnToSteer(pose.getHeading()));
        double powerForTurn = pose.getHeading() * 0.1;
        if (Math.abs(powerForTurn) > 0.1)
        {
            powerForTurn /= Math.abs(powerForTurn);
            powerForTurn *= 0.1;
        }
